...
Torus 3D начинается **немедленно в рамках текущей 0.5**, параллельно с продолжающейся доводкой Cube 3D. Он не является отдельной будущей версией и не ждёт завершения последующего нумерованного этапа.
...
Текущая активная нумерованная версия — **0.5 Cube 3D + Torus 3D**. Реализация Torus 3D начинается сейчас параллельно с текущей работой над 0.5.
...
Добавить полноценные 3D-представления уже работающих Cube Go и Torus Go без изменения правил партии и без отдельных 3D-версий доменного состояния.
...
- Cube 3D;
- Torus 3D;
- переключение между 2D и 3D представлениями в рамках одной и той же партии соответствующей topology;
- полный основной gameplay/endgame cycle в 3D для обеих topology;
- 3D-specific automated/visual regression coverage для Cube и Torus.

Точное пользовательское 3D-поведение определяет `docs/GAME_CUBE_GO.md`; архитектурные границы, reuse и spatial mapping определяет только `docs/ARCHITECTURE.md`.

## Внутренний порядок 0.5

Cube 3D уже является рабочей частью текущего 0.5. Torus 3D начинается немедленно и развивается параллельно с дальнейшей доводкой Cube 3D.

Порядок оставшейся работы:

1. **Torus 3D foundation**;
2. **playable Torus 3D integration**;
3. **Torus 2D ↔ Torus 3D integration**;
4. **joint Cube/Torus 3D polish and regression**;
5. **0.5 release acceptance**.

Содержание технических boundaries каждого пункта определяется текущим `ARCHITECTURE.md`, а пользовательское поведение — `GAME_CUBE_GO.md`.

## Критерий готовности

0.5 достигает технической готовности к пользовательской приёмке, когда полноценную партию каждой поддерживаемой topology можно без изменения игрового смысла продолжать, просматривать, откатывать и завершать в её 2D и 3D представлениях, а общий regression/acceptance gate 0.5 проходит. Фактическое принятие версии подчиняется общему обязательному правилу принятия версий выше.
...
Следующий крупный этап не начинается, пока предыдущий не является рабочей и тестируемой контрольной точкой, кроме явно указанной в текущей версии параллельной разработки Cube 3D и Torus 3D. Переход к следующей нумерованной версии как к принятой предыдущей версии не происходит без явного пользовательского acceptance.
