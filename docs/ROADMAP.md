...
**0.1 Torus 2D → 0.2 Cube 2D → 0.3 Automatic/Assisted alive-dead-seki → 0.5 Cube 3D + Torus 3D**
...
Старая схема «0.3 = Cube 3D» отменена. Полноценные 3D-представления Cube и Torus развиваются в текущей версии 0.5.

Torus 3D начинается **немедленно в рамках текущей 0.5**, параллельно с продолжающейся доводкой Cube 3D. Он не является отдельной будущей версией и должен строиться поверх уже созданной общей 3D-инфраструктуры, а не ждать завершения отдельного последующего этапа.
...
Версии **0.2 Cube 2D** и **0.3 Automatic/Assisted alive-dead-seki** завершены и приняты. 0.3 принята владельцем проекта 16 сентября 2026 года после успешного `0.3 integration / regression acceptance` и пользовательской/UAT-проверки.

Текущая активная нумерованная версия — **0.5 Cube 3D + Torus 3D**. Cube 3D уже является рабочей базой 3D renderer stack; реализация Torus 3D начинается сейчас и идёт поверх той же инфраструктуры с topology-specific geometry/mapping вместо отдельного 3D-стека.
...
# Версия 0.5 — Cube 3D + Torus 3D

## Цель

Добавить полноценные 3D-представления уже работающих Cube Go и Torus Go без изменения правил партии и без отдельных 3D-версий доменного состояния. Torus 3D должен максимально переиспользовать уже готовую Cube 3D инфраструктуру; отдельный параллельный 3D-движок не создаётся.
...
- Cube 3D renderer;
- Torus 3D renderer поверх общей 3D renderer infrastructure;
- после появления 3D topology `Cube` становится default topology при открытии New Game; до 0.5 обязательного default `Cube` нет;
- новая Cube-партия по умолчанию стартует в Cube 3D;
- новая Torus-партия по умолчанию стартует в Torus 3D;
- свободное переключение Cube 2D ↔ Cube 3D и Torus 2D ↔ Torus 3D в рамках одной и той же партии соответствующей topology;
- общий 3D input, screen-space rotation, zoom, picking, camera/light/material/stone presentation и performance behavior с topology-specific настройками только там, где форма поверхности реально различается;
- отображение полного текущего игрового состояния в обоих 3D-представлениях;
- сохранение пространственного orientation/spatial anchor между 2D и 3D представлениями каждой topology;
- feature parity основных игровых и endgame-визуализаций между 2D и 3D режимами;
- 3D-specific automated/visual regression coverage для Cube и Torus.

Точное 3D-поведение и пользовательская семантика определяются `docs/GAME_CUBE_GO.md`; shared 3D renderer boundary и topology-specific spatial mapping определяет только `docs/ARCHITECTURE.md`.

## Внутренний порядок 0.5

Cube 3D уже является готовой рабочей базой. Torus 3D начинается немедленно и использует этот stack как исходную инфраструктуру.

Порядок оставшейся работы:

1. выделить/подтвердить reusable shared 3D renderer core из фактического Cube 3D без изменения его пользовательского поведения;
2. добавить Torus 3D surface geometry и `PointId ↔ surface` mapping как topology-specific слой;
3. подключить существующие shared 3D input/picking/rotation/zoom/stones/materials/light/animations/endgame presentation;
4. реализовать Torus-specific 2D↔3D spatial anchor, reset/navigation mapping и geometry polish;
5. пройти совместный Cube 3D + Torus 3D regression/acceptance.

Работа над Torus 3D не должна ждать отдельного завершения или релиза Cube 3D: эти пункты выполняются в текущем 0.5 цикле, сохраняя уже работающий Cube 3D как regression baseline.

## Критерий готовности

0.5 достигает технической готовности к пользовательской приёмке, когда одну и ту же Cube-партию можно без изменения игрового смысла продолжать, просматривать, откатывать и завершать в Cube 2D и Cube 3D, а одну и ту же Torus-партию — в Torus 2D и Torus 3D; при этом общая 3D инфраструктура переиспользуется без независимых расходящихся Cube/Torus implementations. Фактическое принятие версии подчиняется общему обязательному правилу принятия версий выше.
...
Следующий крупный этап не начинается, пока предыдущий не является рабочей и тестируемой контрольной точкой, кроме явно указанной в текущей версии параллельной разработки Cube 3D и Torus 3D поверх общего 3D stack. Переход к следующей нумерованной версии как к принятой предыдущей версии не происходит без явного пользовательского acceptance.
