...
# 39. Правило оценки кандидатов на адаптацию

Все кандидаты из раздела 38 оцениваются по одной и той же матрице из **25 критериев**. Каждый критерий получает целый балл от `0` до `5`, где `0` всегда означает худший для GoCube результат, а `5` — лучший. Для критериев стоимости, сложности и риска шкала инвертируется: `5` означает низкую стоимость/простую адаптацию/низкий риск.

Оценка должна опираться не на общее впечатление от проекта, а на прочитанный source code, tests, documentation и, где применимо, одинаковый benchmark corpus. Если данных недостаточно, ячейка остаётся `—`, а не заполняется догадкой.

## 39.1. Шкала

```text
0 — unusable / incompatible / absent
1 — very poor; major blockers
2 — weak; substantial adaptation or limitations
3 — acceptable; meaningful work required
4 — strong; minor/moderate adaptation required
5 — excellent fit / best practical case for GoCube
— — not evaluated yet / insufficient evidence
```

## 39.2. 25 критериев

| № | Criterion | Scoring guide: что означает высокий балл |
|---:|---|---|
| 1 | **Fit to GoCube endgame task** | Непосредственно помогает `alive / dead / seki / unresolved`, territory или proof pipeline, а не является общим Go engine без нужной специализации |
| 2 | **Proof quality** | Результат можно превратить в объяснимый deterministic proof; exhaustive/AND-OR semantics предпочтительнее confidence/heuristics |
| 3 | **Cube/Torus adaptability** | Rectangular assumptions локализованы; adjacency легко заменить на `Topology.neighbors(PointId)`; мало edge/corner shortcuts |
| 4 | **Ease of adaptation** | Небольшой и понятный объём изменений, минимум glue code и rewrite |
| 5 | **Architecture quality** | Board/rules/search/scoring/analysis хорошо разделены, зависимости направлены понятно |
| 6 | **Code quality** | Читаемость, types, naming, invariants, отсутствие большого количества hacks и hidden state |
| 7 | **Testability** | Есть unit/regression fixtures, deterministic inputs и возможность изолированно проверять переносимый subsystem |
| 8 | **Observed correctness** | Высокая фактическая корректность на known-answer corpus и низкое число false automatic statuses |
| 9 | **Complex-case coverage** | Покрывает tactics, ladders, nets, snapback, nakade, connections, semeai, seki и другие нужные классы |
| 10 | **Open-boundary robustness** | Не делает ложных выводов, когда группа может выйти наружу, соединиться или изменить локальную границу |
| 11 | **Ko / repetition semantics** | Корректно моделирует ko, repetition/history context и local pass там, где это нужно solver'у |
| 12 | **Performance** | Низкие nodes/time/memory при сопоставимой correctness; хорошо масштабируется на нужные размеры |
| 13 | **Cost predictability / boundedness** | Есть deterministic node/expansion budgets, cancellation или другой безопасный bounded execution model |
| 14 | **Determinism** | Один input даёт один result; randomness отсутствует либо полностью контролируется seed/configuration |
| 15 | **Observability / explainability** | Можно получить PV/proof tree/reason/node count и понять, почему был получен результат |
| 16 | **Partial reuse potential** | Полезный Benson/search/tactical/seki subsystem можно взять отдельно, не импортируя весь engine |
| 17 | **Compatibility with GoCube GameEngine semantics** | Legal moves, play/undo, ko context и state representation можно связать с нашей authoritative rules boundary без второй конкурирующей версии Go |
| 18 | **Browser / TypeScript suitability** | TS/JS/WASM или простой portable core; низкая стоимость browser integration и отсутствие тяжёлого native runtime |
| 19 | **Documentation quality** | Понятны API, algorithms, assumptions, examples и intended semantics |
| 20 | **Project health** | Код не заброшен безнадёжно; repository/build/dependencies доступны и пригодны для изучения/использования |
| 21 | **Maturity / battle testing** | Реальное использование, длительная история, известные пользователи/tests либо иные признаки практической проверки |
| 22 | **License suitability** | Ясная permissive license с низким риском для adaptation; сомнительная/ограничительная license резко снижает балл |
| 23 | **Dependency risk** | Минимум тяжёлых, устаревших, native, neural/model или иных нежелательных dependencies |
| 24 | **Unique value** | Даёт существенную возможность, которой нет у других кандидатов: Torus, special solver, seki logic, search technique и т. п. |
| 25 | **Long-term maintenance cost** | После port/adaptation код будет небольшим, понятным и дешёвым для самостоятельной поддержки GoCube |

Максимальный raw score одного кандидата:

```text
25 criteria × 5 = 125 points
```

Raw total полезен как summary, но **не является единственным решением**. Кандидат с высоким total может быть хуже для конкретного subsystem, чем более узкая библиотека с очень сильными баллами в нужной области. Поэтому итоговое решение должно также указывать предполагаемую роль: `Benson`, `tactical reader`, `full L&D search`, `candidate generator`, `seki/territory`, `Torus`, либо другую конкретную область reuse.

## 39.3. Единая сравнительная таблица

Таблица строится горизонтально. Первые три столбца фиксированы:

1. `№`;
2. `Criterion`;
3. `Scoring guide`.

Начиная с **четвёртого столбца**, каждый следующий столбец принадлежит отдельному кандидату. Новые адаптируемые кандидаты добавляются справа без изменения набора критериев.

| № | Criterion | Scoring guide | Moka | tsumego.js | Sabaki deadstones | OGS score-estimator | goscorer | Go-Variants | Tenuki | Sabaki go-board | Sente | Fuego | Darkforest tsumego |
|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | Fit to GoCube endgame task | Direct relevance to target pipeline | — | — | — | — | — | — | — | — | — | — | — |
| 2 | Proof quality | Strong deterministic/exhaustive proof semantics | — | — | — | — | — | — | — | — | — | — | — |
| 3 | Cube/Torus adaptability | Low rectangular coupling | — | — | — | — | — | — | — | — | — | — | — |
| 4 | Ease of adaptation | Low adaptation effort | — | — | — | — | — | — | — | — | — | — | — |
| 5 | Architecture quality | Clear separation and contracts | — | — | — | — | — | — | — | — | — | — | — |
| 6 | Code quality | Readable, typed/structured, maintainable code | — | — | — | — | — | — | — | — | — | — | — |
| 7 | Testability | Strong deterministic tests/fixtures | — | — | — | — | — | — | — | — | — | — | — |
| 8 | Observed correctness | Strong known-answer results | — | — | — | — | — | — | — | — | — | — | — |
| 9 | Complex-case coverage | Broad relevant endgame coverage | — | — | — | — | — | — | — | — | — | — | — |
| 10 | Open-boundary robustness | Safe handling of escape/connection | — | — | — | — | — | — | — | — | — | — | — |
| 11 | Ko / repetition semantics | Correct history-sensitive handling | — | — | — | — | — | — | — | — | — | — | — |
| 12 | Performance | Low runtime/nodes/memory | — | — | — | — | — | — | — | — | — | — | — |
| 13 | Cost predictability / boundedness | Deterministic bounded execution | — | — | — | — | — | — | — | — | — | — | — |
| 14 | Determinism | Stable reproducible result | — | — | — | — | — | — | — | — | — | — | — |
| 15 | Observability / explainability | Strong diagnostics/proof trace | — | — | — | — | — | — | — | — | — | — | — |
| 16 | Partial reuse potential | Easy subsystem extraction | — | — | — | — | — | — | — | — | — | — | — |
| 17 | GameEngine compatibility | Low risk of duplicated rule semantics | — | — | — | — | — | — | — | — | — | — | — |
| 18 | Browser / TypeScript suitability | Low browser integration cost | — | — | — | — | — | — | — | — | — | — | — |
| 19 | Documentation quality | Clear technical documentation | — | — | — | — | — | — | — | — | — | — | — |
| 20 | Project health | Usable current repository/build | — | — | — | — | — | — | — | — | — | — | — |
| 21 | Maturity / battle testing | Evidence of practical validation | — | — | — | — | — | — | — | — | — | — | — |
| 22 | License suitability | Clear permissive reuse terms | — | — | — | — | — | — | — | — | — | — | — |
| 23 | Dependency risk | Few/light/safe dependencies | — | — | — | — | — | — | — | — | — | — | — |
| 24 | Unique value | Non-duplicated capability for GoCube | — | — | — | — | — | — | — | — | — | — | — |
| 25 | Long-term maintenance cost | Low future ownership cost | — | — | — | — | — | — | — | — | — | — | — |
|  | **Raw total / 125** | Sum of 25 scores | — | — | — | — | — | — | — | — | — | — | — |

## 39.4. Правило заполнения

Оценки заполняются поэтапно, чтобы не смешивать предположения с измерениями:

```text
Stage 1 — Source/code audit
Stage 2 — Same-corpus functional benchmark
Stage 3 — Small GoCube adaptation spike for leading candidates
Stage 4 — Final score revision and reuse decision
```

После каждого этапа разрешено пересматривать предыдущие баллы, если появились более сильные evidence. Финальная таблица должна отражать **последнюю доказанную оценку**, а не историю изменения мнения; история остаётся в Git.
