...
Legacy restore compatibility принадлежит shared application restore lifecycle, а не Renderer/Presentation. После того как persisted snapshot прошёл единый trust boundary и `GameSession` восстановлен, application/shared facade до публикации восстановленного `ActiveGame` в UI обязан проверить ровно условие `phase = endgame && endgameReview = null` и `await` существующий `GameSession.resumeEndgame()`. Для `playing`, `finished` и `endgame` с уже persisted review этот resume не запускается. Сохранённый `EndgameReviewState` и его пользовательские решения восстанавливаются как проверенное session state без повторного classifier run. Review, созданный compatibility-resume, использует ту же fail-soft classifier/proposal boundary из предыдущего абзаца и сразу сохраняется через обычный ordered autosave/persistence path; поэтому следующий reload восстанавливает его напрямую без повторного анализа. Torus/Cube-specific controllers могут только делегировать этот hook shared facade и не владеют отдельной restore/endgame orchestration.
...
Bot-aware history operation не создаёт `botHistory`, `botUndoStack` или другой timeline. Orchestrator определяет границу последнего человеческого решения только по authoritative `GameSessionSnapshot.history` и выполняет требуемые один или два шага через существующие controller/`GameSession.undo()`/`redo()`. Opening bot move при Human White не образует самостоятельную human-decision boundary, потому что перед ним в authoritative timeline нет human action. Любая такая Undo mutation меняет authoritative session revision, поэтому уже отправленный `BotTurnCoordinator` request становится stale по существующему position-identity proof и не может применить поздний ответ.

Redo также читает только authoritative redo-future. После восстановления human state orchestrator проверяет, существует ли следующий redo-state с уже сыгранным bot action: если да, он восстанавливается обычным controller/`GameSession.redo()` без нового `selectMove()`; если нет, запускается обычный новый `BotTurnCoordinator` из восстановленной authoritative History. Undo очищает bot failure/retry lifecycle для отменённой позиции; error state не хранится как history и не восстанавливается отдельно.

Для каждой controller/session identity допускается максимум один **актуальный** in-flight bot request. Пока request остаётся актуальным, lifecycle имеет состояние `bot-thinking`; новые human place/Pass и второй bot launch для той же позиции блокируются application-level. History mutation, делающая request stale, немедленно лишает его логического single-flight slot: transport completion физически может прийти позже, но его completion/error/presentation callbacks fenced по identity попытки и не имеют права менять восстановленную позицию или новый lifecycle. Поэтому после Undo → Redo human-only допускается новый актуальный MCTS до физического завершения уже инвалидированного старого transport request. Idle `playing` state выводится из authoritative `currentPlayer` как human turn либо bot turn; `endgame` и `finished` выводятся из обычного `GamePhase`. Это orchestration state, а не второй `GameState`.

Failure текущего актуального bot turn переводит orchestrator этой же controller/session identity в `error`, не откатывая уже принятый human move и не создавая fallback Pass. `retryBotTurn()` разрешён только для сохранившейся `playing` позиции, где authoritative `currentPlayer` всё ещё принадлежит боту; retry заново вызывает `BotTurnCoordinator`, поэтому Protocol V1 history снова строится из текущей полной authoritative History. Ошибка или завершение obsolete request предыдущего controller либо history-invalidated attempt не имеет права переводить актуальную партию в error.

Замена owning controller/session не отменяет stale proof внутри `BotTurnCoordinator`: старый response не применяется к новой партии. Single-flight scoped к controller identity, а внутри controller — к актуальной attempt identity, поэтому зависший stale запрос не превращается в глобальный lock новой партии или восстановленной history position. Независимо от исхода `applied | stale | error` obsolete lifecycle не должен оставлять актуальную партию навсегда в `bot-thinking`.
...
Presentation не вызывает bot-specific controller напрямую. Для dispatch пользовательских `placeStone`/`pass`/`undo`/`redo` используется явный `GameInteractionBoundary`: в обычной партии он делегирует существующему controller, а в Human-vs-Bot партии — `BotGameOrchestrator`. Boundary не владеет `GameState`, History, captures, redo-future или turn state и не является второй gameplay model.
...
4. Orchestrator устанавливает per-controller current-attempt `bot-thinking` и вызывает `BotTurnCoordinator.playBotTurn()`.
...
6. `applied` возвращает lifecycle к human turn, если authoritative phase остаётся `playing`; `stale` ничего не мутирует; failure текущей актуальной попытки оставляет позицию как есть и делает доступным retry.
7. Retry повторяет шаги 4–5 из **текущей** authoritative History, не из сохранённой копии предыдущего request.
8. Bot-aware Undo/Redo вычисляет только число существующих history-steps, относящихся к последнему human decision, и выполняет их существующими controller/`GameSession` history commands; собственный bot timeline не создаётся.
9. Undo инвалидирует текущую bot attempt через mutation authoritative session; поздний completion старой attempt fenced и не мутирует position/lifecycle. Redo уже существующего bot state идёт через redo-future без AlphaZero; если в future есть только human state, после его восстановления запускается обычный новый bot turn.
10. Любой переход в `endgame`/`finished` прекращает bot-request lifecycle и оставляет дальнейшую обработку существующим session/endgame flow.
...
- legacy `endgame` snapshot без persisted review после успешного trust-boundary restore проходит shared compatibility-resume до публикации в UI, получает все реальные logical stone groups, autosave-ит созданный review и на следующем reload не запускает classifier повторно; snapshot с уже сохранённым review сохраняет пользовательские решения без reanalysis, а classifier exception/invalid proposal при resume приводит к обычному unresolved fallback;
...
`BotGameOrchestrator` headless integration coverage обязана проверять: Human Black `human move → bot move → human turn`; Human White с автоматическим первым Black bot move; bot `place` и `pass`; human Pass с последующим bot response; блокировку human input и второго **актуального** bot launch во время `bot-thinking`; сохранение accepted human move при AlphaZero failure; `retryBotTurn()` из актуальной полной authoritative History; stale response после замены controller/session без mutation; Human Black `human → bot → Undo → Redo`; Human White, где opening Black bot move не отменяется отдельно; Undo во время `bot-thinking` и отсутствие mutation от позднего stale response; Undo после bot failure с очисткой retry/error state; Redo уже существующей human+bot пары без нового `/v1/move`; Redo human-only state после interrupted bot turn с новым обычным `/v1/move`; Pass/endgame через bot-aware Undo/Redo; clearing redo-future после нового human action; normal endgame после второго Pass; отсутствие влияния на обычную local human-vs-human игру. Минимальный Chromium E2E acceptance отдельно доказывает основной `human → bot → Undo → Redo` path и отсутствие второго bot request при восстановлении готовой пары. Базовый lifecycle acceptance также должен доказать две последовательные пары `A → B`, `C → D` и Protocol V1 histories `[A]`, затем `[A, B, C]`.
...
- хранить параллельные `botBoard`/`botHistory`/`botGameState`/`botUndoStack`/`botRedoStack` для Human-vs-Bot lifecycle или повторять projection/Protocol V1 logic внутри `BotGameOrchestrator`;
- запускать второй **актуальный** AlphaZero MCTS для той же controller/session position, пока предыдущий актуальный bot request остаётся in-flight; history-invalidated stale request не считается владельцем текущего single-flight slot, но его поздний completion обязан быть fenced;
- откатывать accepted human move либо подставлять fallback Pass из-за ошибки последующего bot request вне явной пользовательской Undo-команды;
...
- для настоящей Human-vs-Bot партии `BotGameOrchestrator` после accepted human action проверяет authoritative turn/phase и запускает один актуальный `BotTurnCoordinator`; coordinator каждый раз строит AlphaZero position из authoritative History и возвращает proposal только через обычный `GameSession → GameEngine` path;
- bot-aware Undo/Redo остаётся orchestration над той же authoritative History: orchestrator вызывает существующие controller/`GameSession` history commands, восстанавливает готовый bot state из redo-future без AlphaZero либо запускает новый bot turn только когда такого state нет; отдельного bot timeline нет;
- `BotGameOrchestrator` хранит только per-controller current-attempt и retry/error lifecycle; history mutation инвалидирует старую attempt, а late stale response fenced и не создаёт параллельную board/history;
