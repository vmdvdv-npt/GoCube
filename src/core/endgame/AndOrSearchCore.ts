...
export interface AndOrSearchResult {
  ...
  readonly outcome: AndOrSearchOutcome;
  readonly unknownReason: AndOrUnknownReason | null;
  /** Unique, non-transposition nodes admitted by the deterministic node budget. */
...
const cacheKeyFor = (nodeType: AndOrNodeType, nodeKey: string): string =>
  `${nodeType}\u0000${nodeKey}`;

const unknownRank: Readonly<Record<AndOrUnknownReason, number>> = Object.freeze({
  budget: 3,
  incomplete: 2,
  cycle: 1,
});

const selectUnknownReason = (
  reasons: readonly AndOrUnknownReason[],
): AndOrUnknownReason => {
  let selected: AndOrUnknownReason = 'cycle';
  for (const reason of reasons) {
    if (unknownRank[reason] > unknownRank[selected]) selected = reason;
  }
  return selected;
};
...
const leafTrace = (
  ...
  Object.freeze({
    ...
    unknownReason,
    source,
    children: EMPTY_TRACE_CHILDREN,
  });

const freezeTraceChild = (
  move: string,
  verdict: SearchVerdict,
): AndOrProofTraceChild =>
  Object.freeze({
    move,
...
const search = <State>(
  ...
  if (cached) {
    ...
    return Object.freeze({
      outcome: cached,
      unknownReason: null,
      trace: leafTrace(nodeKey, nodeType, cached, null, 'transposition'),
    });
  }

  if (activePath.has(cacheKey)) {
    return unknownVerdict(nodeKey, nodeType, 'cycle', 'cycle');
  }

  if (runtime.exploredNodes >= runtime.maxNodes) {
  ...
  if (nodeType === 'or') {
    for (const child of expansion.children) {
      const verdict = search(runtime, child.state, nextPath);
      ...
      if (verdict.outcome === 'unknown' && verdict.unknownReason) {
        unknownReasons.push(verdict.unknownReason);
      }
    }
    ...
    if (unknownReasons.length > 0) {
      return unknownVerdict(
        nodeKey,
        nodeType,
        selectUnknownReason(unknownReasons),
        'expanded',
        traceChildren,
      );
    }
...
export const runAndOrSearch = <State>(
  ...
  const verdict = search(runtime, root, new Set());
  return Object.freeze({
