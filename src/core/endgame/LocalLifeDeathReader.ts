...
export const LOCAL_LIFE_DEATH_ALGORITHM = 'local-life-death-v1';
...
export interface LocalLifeDeathOptions {
  ...
  /** Reuses the Work 5A deterministic relevance-zone safety cap. */
...
const targetSnapshot = (
  ...
  const survivingStone = runtime.crucialStones.find(
    (point) => state.board[point] === runtime.targetColor,
  );
  if (!survivingStone) {
    return Object.freeze({ graph, group: null, bensonAlive: false });
  }
  ...
  const bensonAlive =
    group !== null &&
    proveBensonPassAlive(state.board, runtime.topology, graph, runtime.targetColor).has(group.key);

  return Object.freeze({ graph, group, bensonAlive });
};

const groupAtCrucialStone = (
  runtime: LocalRuntime,
  state: GameState,
): StoneGroup | null => {
  const survivingStone = runtime.crucialStones.find(
...
const isStillInsideCertifiedZone = (
  runtime: LocalRuntime,
  state: GameState,
): boolean => {
  const snapshot = targetSnapshot(runtime, state);
  if (!snapshot.group) return true;
  if (snapshot.bensonAlive) return true;

  const currentZone = buildRelevanceZone(snapshot.group, state.board, runtime.topology, {
    maxPoints: runtime.maxZonePoints,
  });
  return (
    currentZone.outcome === 'bounded' &&
    currentZone.points.every((point) => runtime.zonePointSet.has(point))
  );
};

/**
 * A move that creates an immediate simple-ko recapture cannot be used as an
 * unconditional life/death proof. The local solver intentionally does not model
 * external ko threats, so the entire continuation fails closed as ko-dependent.
 */
const createsSimpleKo = (
  ...
  for (const point of runtime.topology.points()) {
...
const makeUncertain = (
  ...
  mover: StoneColor,
): LocalSearchState => Object.freeze({ kind: 'uncertain' as const, reason, mover });
...
const childOrderingMetric = (
  ...
  const group = groupAtCrucialStone(runtime, childState);
  const liberties = group?.liberties.length ?? 0;
...
const buildAdapter = (runtime: LocalRuntime): AndOrSearchAdapter<LocalSearchState> => ({
  ...
  expand: (state) => {
    ...
    // lifts immediate simple-ko, exactly as a real intervening move would.
    children.push(
      Object.freeze({
        move: 'tenuki',
        state: makePosition(
...
const runOrder = (
  ...
  const search = runAndOrSearch(
    makePosition(
      ...
      mover,
      Object.freeze({ kind: 'root-unknown' as const }),
    ),
