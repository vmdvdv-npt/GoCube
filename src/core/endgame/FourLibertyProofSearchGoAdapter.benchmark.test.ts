...
const BENCHMARK_ENABLED =
  (globalThis as BenchmarkGlobal).process?.env?.ENGINE2_FOUR_LIBERTY_BENCHMARK === '1';
const WARMUP_RUNS = 2;
...
const makeFixture = (topology: Topology): Fixture => {
  ...
  if (!targetGroup || targetGroup.liberties.length !== 4) {
    throw new Error(`Expected exactly four target liberties for ${target}`);
  }

  return Object.freeze({
    state,
    target,
    emptyPoints: points.length - 1,
  });
};
...
const runCase = (benchmarkCase: BenchmarkCase, workload: Workload): void => {
  ...
  if (attacker) {
    ...
    expect(baseline.moveKeys).toHaveLength(4);
  ...
  console.log(
    `ENGINE2_FOUR_LIBERTY_BENCHMARK_RESULT ${JSON.stringify({
...
benchmarkDescribe('FourLibertyProofSearchGoAdapter E2-6 performance gate', () => {
