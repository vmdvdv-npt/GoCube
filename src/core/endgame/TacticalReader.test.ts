...
import { TACTICAL_READER_ALGORITHM, readTacticalCapture } from './TacticalReader';
...
const bothOrdersKill = (target: EndgameStoneString, state: GameState, topology: Topology): boolean =>
  readTacticalCapture(target, state, topology, { firstPlayer: 'attacker' }).outcome === 'proved-kill' &&
  readTacticalCapture(target, state, topology, { firstPlayer: 'defender' }).outcome === 'proved-kill';

const asPlaying = (state: GameState): GameState =>
  Object.freeze({ ...state, phase: 'playing' as const, consecutivePasses: 0 });

describe('TacticalReader proof safety', () => {
  it('proves an immediate capture even when the defender receives the first local move', () => {
    ...
    const state = makeState(topology, {
      '0,0': 'white', '0,1': 'black', '1,1': 'black', '2,0': 'black',
    });
    ...
    const defenderFirst = readTacticalCapture(target, state, topology, { firstPlayer: 'defender' });

    expect(attackerFirst.algorithm).toBe(TACTICAL_READER_ALGORITHM);
    expect(attackerFirst.outcome).toBe('proved-kill');
    expect(attackerFirst.principalVariation).toEqual(['1,0']);
    expect(defenderFirst.outcome).toBe('proved-kill');
  });

  it('does not call an atari dead when the defender can extend into open space', () => {
    const topology = new GridTopology(3);
    const state = makeState(topology, {
      '1,1': 'white', '0,1': 'black', '1,0': 'black', '2,1': 'black',
    });
    ...
    expect(bothOrdersKill(target, state, topology)).toBe(false);
  });

  it('proves a short multi-step forced capture', () => {
    const topology = new GridTopology(3);
    const state = makeState(topology, {
      '0,0': 'white', '1,1': 'black', '2,0': 'black', '0,2': 'black',
    });
    const target = targetAt(state, topology, '0,0');
    expect(bothOrdersKill(target, state, topology)).toBe(true);
    const result = readTacticalCapture(target, state, topology, { firstPlayer: 'attacker' });
    expect(result.principalVariation.length).toBeGreaterThanOrEqual(2);
  });

  it('reads a ladder as a repeated forced-atari sequence', () => {
    const topology = new GridTopology(5);
    const state = makeState(topology, {
      '1,1': 'white', '0,0': 'black', '0,1': 'black', '1,0': 'black',
      '2,0': 'black', '3,0': 'black', '4,0': 'black',
    });
    const target = targetAt(state, topology, '1,1');
    const result = readTacticalCapture(target, state, topology, { firstPlayer: 'attacker' });
  ...
  it('finds a net move outside the target current liberties', () => {
    const topology = new GridTopology(4);
    const state = makeState(topology, {
      '0,0': 'white', '1,0': 'black', '2,0': 'white', '3,0': 'white',
      '0,1': 'black', '1,1': 'white', '2,1': 'white', '2,2': 'black',
      '3,2': 'black', '0,3': 'black', '1,3': 'black', '2,3': 'black',
    });
  ...
  it('handles snapback by allowing the defender counter-capture', () => {
    const topology = new GridTopology(4);
    const state = makeState(topology, {
      '0,0': 'white', '3,0': 'black', '0,1': 'white', '1,1': 'black',
      '3,1': 'white', '2,2': 'black', '3,2': 'black', '0,3': 'white',
      '1,3': 'white', '2,3': 'black', '3,3': 'black',
    });
    ...
    if (!first.ok) return;
    const reply = engine.placeStone(first.state, '1,2', 'white', { previousBoard: state.board });
  ...
  it('turns node/deadline exhaustion and open escape into unknown', () => {
    const topology = new GridTopology(5);
    const state = makeState(topology, { '2,2': 'white' });
    const target = targetAt(state, topology, '2,2');

    expect(readTacticalCapture(target, state, topology, { firstPlayer: 'attacker' }).outcome).toBe('unknown-boundary');
    expect(readTacticalCapture(target, state, topology, {
      firstPlayer: 'attacker', maxNodes: 0,
