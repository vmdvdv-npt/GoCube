...
export interface BotTurnGameController {
  ...
  pass(): Promise<BotTurnControllerActionResult>;
}
...
export class BotTurnCompatibilityError extends Error {
  readonly name = 'BotTurnCompatibilityError';

  constructor(
    readonly reasonCode: BotTurnCompatibilityReason,
...
export class BotTurnUnavailableError extends Error {
  readonly name = 'BotTurnUnavailableError';
}

type PositionIdentity = Readonly<{
...
export class BotTurnCoordinator {
  ...
  constructor(private readonly options: BotTurnCoordinatorOptions) {
    if (options.checkpointId.trim().length === 0) {
      throw new Error('Bot checkpointId must be non-empty');
    }
    if (!Number.isSafeInteger(options.mctsSimulations) || options.mctsSimulations <= 0) {
      throw new Error(
        `Bot MCTS simulations must be a positive safe integer, got ${String(options.mctsSimulations)}`,
      );
    }
  ...
  async playBotTurn(): Promise<BotTurnResult> {
    ...
    const identity: PositionIdentity = Object.freeze({
      controller,
      sessionRevision: sessionRevision(snapshot),
    ...
    const response = await this.options.gateway.selectMove(
      Object.freeze({
        ...
        mctsSimulations: this.options.mctsSimulations,
        position: projectGameSessionToAlphaZeroPosition(snapshot, this.options),
      }),
    );

    const currentController = this.options.controller();
    const latestSnapshot = currentController.snapshot();
    const latestState = currentState(latestSnapshot);
    if (!this.matchesPositionIdentity(identity, currentController, latestSnapshot)) {
      return Object.freeze({ status: 'stale' as const, requestId });
    }
    ...
    const result =
      response.action.type === 'place'
        ...
        : await currentController.pass();

    if (!result.accepted) {
      throw new BotTurnCompatibilityError(
        'action-rejected',
        requestId,
        `GoCube rejected AlphaZero ${this.describeAction(response.action)}${
  ...
  private matchesPositionIdentity(
    ...
    return (
      ...
      sessionRevision(snapshot) === expected.sessionRevision &&
      state.moveNumber === expected.moveNumber &&
      state.currentPlayer === expected.currentPlayer &&
      state.phase === 'playing'
    );
  }

  private assertResponseColor(
    response: AlphaZeroSelectedMove,
    currentPlayer: StoneColor,
    requestId: string,
  ): void {
  ...
  private describeAction(action: AlphaZeroAction): string {
    return action.type === 'place' ? `place(${action.pointId})` : 'pass';
  }
}
