...
describe('Cube 2D logical 4x3 placement field', () => {
  ...
  it('builds a 4x3 cross with exactly six occupied faces and six empty slots', () => {
    const layout = createCube2DLayout(new CubeOrientation(), 3);
    ...
    expect(layout.cells.map((cell) => cell.face)).toHaveLength(6);
  ...
  it('keeps every visible cross seam compatible with CubeTopology across all 24 orientations', () => {
    ...
    for (const orientation of orientations) {
      const layout = createCube2DLayout(orientation, 3);
      const middle = layout.rows[1].map(requireCell);
      const top = requireCell(layout.rows[0][1]);
      const center = requireCell(layout.rows[1][1]);
  ...
  it('rebuilds the six-face cross around a new orientation without creating copies', () => {
